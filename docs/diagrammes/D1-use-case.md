...
flowchart LR
    ...
    subgraph SYSTEM["Attendance & Peer Review Tracker"]
        ...
        UC4(("UC4 Assign two distinct reviewers among attendees - EF4"))
        ...
        UC9(("UC9 View retained grade, provisional flag and comments, anonymous reviewers - EF9"))
...
- Extension points: UC2 includes rate limiting after 5 failed attempts (RG3, → 429 TOO_MANY_ATTEMPTS);
  UC3 requires prior attendance (RG14, → 400 PRESENCE_REQUISE).
- Step 3 change of need: UC4 now draws **two** distinct reviewers (RG5 revised); UC11
  (amend a review) was removed — EF11 is the scope sacrificed for this change (CDC §10).
